...
```json
{
  "type": "urn:deskify:error/policy-denied",
...
6. API 302s to the app's Universal Link / App Link:
        https://app.deskify.example/auth/return?code=…
...
logs. Universal Links are used rather than a custom `deskify://` scheme because a custom scheme can
...
```
deskify:v1:<resource_id>:<key_version>:<base64url HMAC-SHA256(resource_id|key_version, org_secret)>
```
...
setting `DESKIFY_ENVIRONMENT=dev` — would also drop `ssl="require"` and re-enable
...
| D17 | Database credentials guarded on strength, not on host | §15.4 — a localhost database in prod is legitimate; a `deskify:deskify` one is not |
